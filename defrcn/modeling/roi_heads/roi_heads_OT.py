...
class CommonalityROIHeads(ROIHeads):
    ...
    def getStatistics(self, ):
        ...
        base_mean = torch.stack(base_mean, dim=0)  # 【15，2048】
        ...
        novel_mean = torch.stack(novel_mean, dim=0)  # [5,2048]
        novel_std = torch.stack(novel_std, dim=0)  # [5,2048]
            ...
            if int(storage.iter) >= self.warmup_distill:
                ...
                for gtBoxFea, gtBoxcls in zip(gtBox_feature_pooled_s, gtBox_classes):
                    if gtBoxcls in self.novel_index:
                        ...
                        gtNovStd.append(novel_std[gtBoxcls-60])  #voc:15
                ...
                nv = torch.ones(20, device= dists_novall.device)  #到20个类距离（分配）
