...
class SinkhornDistance(torch.nn.Module):
    ...
    def forward(self, mu, nv, C): #C:[15,5]
        u = torch.ones_like(mu)  #15个1
        v = torch.ones_like(nv) #20个1

        # Sinkhorn iterations
        for i in range(self.max_iter):
            v = self.eps * \
                (torch.log(                      #trans后[5,15]->logsumexp后[5]
                    nv + 1e-8) - torch.logsumexp(self.M(C, u, v).transpose(-2, -1), dim=-1)) + v
            u = self.eps * \
                (torch.log(
    ...
    def label_and_sample_proposals(self, proposals, targets):
        ...
        for proposals_per_image, targets_per_image in zip(proposals, targets):
            ...
            matched_idxs, matched_labels = self.proposal_matcher(
                match_quality_matrix  # 与哪个gt_label匹配，matched_labels:iou>0.5-->1; ..0
            )
...
class CommonalityROIHeads(ROIHeads):
    ...
    def __init__(self, cfg, input_shape):
        ...
        self.res5, out_channels = self._build_res5_block(cfg)
        output_layer = cfg.MODEL.ROI_HEADS.OUTPUT_LAYER  # base train:"DoubleFastRCNNOutputLayers"
        self.box_predictor = ROI_HEADS_OUTPUT_REGISTRY.get(output_layer)(
            cfg, out_channels, self.num_classes, self.cls_agnostic_bbox_reg
        )
        # 不同于DeFRCN
        ...
        # create the queue
        self.queue_len = cfg.MODEL.ROI_HEADS.QUEUE_LEN  # 2048
        if self.memory:
            self.register_buffer("queue_s", torch.zeros(self.num_classes, self.queue_len,
                                                        out_channels))  # torch.Size([20, 2048, 2048])
            self.register_buffer("queue_l", torch.zeros(self.num_classes, self.queue_len,
                                                        out_channels))  # torch.Size([20, 2048, 2048])
            self.register_buffer("queue_ptr", torch.zeros(self.num_classes, dtype=torch.long))  # Tensor:(20,)
            self.register_buffer("queue_full", torch.zeros(self.num_classes, dtype=torch.long))  # Tensor:(20,)
        ...
        SINKHORN_EPS = 0.1
        SINKHORN_ITER = 50
        self.sinkhorn = SinkhornDistance(eps=SINKHORN_EPS,
                                         max_iter=SINKHORN_ITER)
        # self.getStatistics = getStatistics(self, novelgtBoxFeas, gtBox_classes)
    ...
    def predict_prototype(self, feature_pooled_s, feature_pooled_l, gt_classes):
        ...
        prototypes_s = []
        prototypes_l = []
        for i in range(self.num_classes):  # 20个类
            ...
            else:  # queue_s:[20,bs*512,2048]; queue_str:tensor(20):每个类fg个数。
        ...
        prototypes_s = torch.stack(prototypes_s, dim=0)  # 类原型（20，2048）
        prototypes_l = torch.stack(prototypes_l, dim=0)

        # 每个proposal特征，与原型相似度。(512*bs,2048); (20,2048)
        predict_cosine_s = F.cosine_similarity(feature_pooled_s[:, None], prototypes_s[None, :], dim=-1)  # (4096,20)
        predict_classes_s = predict_cosine_s.new_full(predict_cosine_s.size(), -1.0)  # -1填充，
        predict_classes_s[:, self.novel_index] = predict_cosine_s[:, self.novel_index]  # novel类位置，用相似度更新

        # # 欧氏距离
        # predict_dis_s = F.cosine_similarity(
        #     feature_pooled_s[:, None].expand(feature_pooled_s.shape(0), prototypes_s.shape(0), prototypes_s.shape(1)),
        #     prototypes_s.expand(feature_pooled_s.shape(0), prototypes_s.shape(0), prototypes_s.shape(1)))  # (4096,20)
        # predict_classes_s2 = predict_dis_s.new_full(predict_dis_s.size(), -1.0)  # -1填充，
        # predict_classes_s2[:, self.novel_index] = predict_dis_s[:, self.novel_index]  # novel类位置，用相似度更新
        ...
        for i in range(len(predict_classes_s)):  # 4096
            if gt_classes[i] == self.num_classes or gt_classes[i] < 0:
                continue
            predict_classes_s[i, gt_classes[i]] = predict_cosine_s[i, gt_classes[i]]  # 用相似度填充整个预测类得分。
    ...
    def getStatistics(self, ):
        # uniq_c = torch.unique(gtBox_classes)
        kth = 2
        base_features = self.queue_s[self.base_index]
        n_base = len(self.base_index)
        n_novel = len(self.novel_index)
        alpha = 1.0  # hyper param;
        beta = 0.0  # variance hyper param
        ...
        base_mean = torch.stack(base_mean, dim=0)  # [60,2048] /【15，2048】
        base_std = torch.stack(base_std, dim=0)
        ##add OT
        novel_mean, novel_std, dists = [], [], []
        for c in self.novel_index:  # 新类中每个类
            if self.queue_full[c]:
                c_mean = self.queue_s[c].mean(dim=0)
                c_std = self.queue_s[c].var(dim=0, unbiased=False)
                c_std = c_std * self.queue_len / (self.queue_len - 1)
                ...
                if self.queue_ptr[c] > 1:
                    c_std = c_std * self.queue_ptr[c] / (self.queue_ptr[c] - 1)
            novel_mean.append(c_mean)
            novel_std.append(c_std)
        novel_mean = torch.stack(novel_mean, dim=0)  # [20,2048]/[5,2048]
        novel_std = torch.stack(novel_std, dim=0)  # [20,2048]/[5,2048]

        return base_mean, base_std, novel_mean, novel_std
    ...
    def forward(self, images, features, proposals, targets=None):
        ...
        proposal_boxes = [x.proposal_boxes for x in proposals]  # list[bs]:512/
        box_features = self._shared_roi_transform(
            [features[f] for f in self.in_features], proposal_boxes
        )  # torch.Size([1024, 2048, 4, 4])
        feature_pooled = box_features.mean(dim=[2, 3])  # pooled to 1x1  torch.Size([1024, 2048])
        ...
        if self.training:
            # box features提取
            gt_boxes = [x.gt_boxes for x in targets]
            gt_box_features = self._shared_roi_transform(  # 'res5' torch.Size([17, 2048, 4, 4]);5个实例
                [features[f] for f in self.in_features], gt_boxes
            )
            gtBox_feature_pooled = gt_box_features.mean(dim=[2, 3])  # pooled to 1x1
            gtBox_feature_pooled_s = F.relu(self.fc_s(gtBox_feature_pooled))  # [17,2048]
            gtBox_feature_pooled_l = F.relu(self.fc_l(gtBox_feature_pooled))  # [17,2048]
            # self.gt_boxes = box_type.cat([p.gt_boxes for p in proposals])
            gtBox_classes = cat([p.gt_classes for p in targets], dim=0)  # [17]
        ...
        if self.training: 
            #在当前batch样本未加入queue之前，计算校正新类gtBox的统计特性：
            storage = get_event_storage()
            if int(storage.iter) >= self.warmup_distill:
                # if self.augmentation:
                # 对每个新类的gt_box
                gt_classes = outputs.gt_classes
                base_mean, base_std, novel_mean, novel_std = self.getStatistics()
                cls_mean = torch.cat([base_mean, novel_mean], 0)
                cls_std = torch.cat([base_std,novel_std], 0)
                kth = 2
                alpha = 0.8
                beta =0.
                num_samples = 100
                ...
                dists_gtNovBase = []      
                gtBox_cal_mean = []   
                for gtBoxFea, gtBoxcls in zip(gtBox_feature_pooled_s, gtBox_classes):
                    t0 = []
                    if gtBoxcls in self.novel_index:
                        #self.novel_index = [0, 1, 2, 3, 4, 5, 6, 8, 14, 15, 16, 17, 18, 19, 39, 56, 57, 58, 60, 62]
                        gtNovFeas.append(gtBoxFea)
                        gtNovClses.append(gtBoxcls)
                        #查看与所有类距离：
                        dists_gtNov2Allcls = torch.norm(gtBoxFea - cls_mean, p=2, dim=1)
                        dists_gtNovelbaseCls = torch.norm(gtBoxFea - base_mean, p=2, dim=1)
                        dists_gtnovall.append(dists_gtNov2Allcls/dists_gtNov2Allcls.max())
                        dists_gtNovBase.append(dists_gtNovelbaseCls/dists_gtNovelbaseCls.max())
                        # dists_gtnovall2.append(torch.exp(dists_gtNov2Allcls)/torch.sum(torch.exp(dists_gtNov2Allcls)))
                       
                        t0.append(novel_std[self.novel_index.index(gtBoxcls)])
                        gtNovStd.append(t0[0])  #voc: 15
                        
                        # gtBox_mu = torch.add(gtBoxFea,novel_mean[gtBoxcls-15])/2
                        # gtBox_cal_mean.append(gtBox_mu)
                gtNovFeas = torch.stack(gtNovFeas)
                gtNovClses = torch.stack(gtNovClses)    
                gtNovStd =  torch.stack(gtNovStd)     
                dists_novall = torch.stack(dists_gtnovall, dim=0)     
                # dists_novBase = torch.stack(dists_gtNovBase, dim=0)  #[4,15]

                _, index = dists_novall.sort(dim=1)
                # _, index2 = dists_novBase.sort(dim=1)
                num_gtNovFeas = len(gtNovFeas)
                mu = torch.ones(num_gtNovFeas,  device = dists_novall.device)   #gtNovFeas个数
                nv = torch.ones(80, device= dists_novall.device)  #到20个类距离（分配）

                # mu = gtNovFeas.new_ones(gtNovFeas.size()[0])
                # nv = dists_novall.new_ones(dists_novall.size()[1])
                
                _, P_nov2AllCls = self.sinkhorn(mu, nv, dists_novall) #[gtNovFeas,20]
                # xx,idx = P_nov2AllCls.sort(1,True)
                # _, P_nov2AllCls2 = self.sinkhorn(mu, nv, dists_novall2) #[gtNovFeas,20]

                # P_nov2AllCls[:,15:] = P_nov2AllCls[:,15:]*0.8

                #直接矩阵相乘得到gt的均值 [2,80]    [80,2048]
                cal_mu = torch.matmul(P_nov2AllCls, cls_mean) #[4,20] [20,2048]->[4,2048]
                # # topk,mask?
                calibrated_mean = alpha* gtNovFeas + (1-alpha) *cal_mu
                # calibrated_mean = gtBox_cal_mean
                
                # P_nov2AllCls[:,60:] = P_nov2AllCls[:,60:]*0.9  #
                cal_std = torch.matmul(P_nov2AllCls, cls_std)
                calibrated_std = beta * gtNovStd +(1-beta)* cal_std

                for i in range(len(gtNovFeas)):
                    # if np.random.rand() < 0.5:
                    #     continue

                    # #用新类均值：
                    # calibrated_gtMean = novel_mean[gtBoxcls-15]
                    # #用相近基类均值：
                    # calibrated_gtMean_2 = (gtBoxFeas+base_mean[index[:kth]].mean(dim=0))/(kth+1)
                    # # 用相近基类方差：= base_std[index[:kth]].mean(dim=0)

                    # ids = index2[i]
                    # calibrated_mean = (torch.cat((gtNovFeas[i].unsqueeze(0), base_mean[ids[:kth]]),0)).mean(dim=0)
                    # calibrated_std = base_std[ids[:kth]].mean(dim=0)

                    #特征生成
                    univariate_normal_dists = distributions.normal.Normal(
                        calibrated_mean[i], scale=torch.sqrt(calibrated_std[i]))  # -len(self.base_index)
                    # univariate_normal_dists = distributions.normal.Normal(
                    #     calibrated_mean, scale=torch.sqrt(calibrated_std))  # -len(self.base_index)
                    feaures_rsample = univariate_normal_dists.rsample(
                        (num_samples,))
                    classes_rsample = gt_classes.new_full((num_samples,), gtNovClses[i])
                ...
                if len(new_features) == 0:
                    ...
                    new_classes = torch.cat(new_classes)  # torch.Size([40, 2048]);
            ...
            if self.memory:
                with torch.no_grad():
                    ...
                    # pad_size = self.cfg.MODEL.ROI_HEADS.BATCH_SIZE_PER_IMAGE \
                    #            * self.cfg.SOLVER.IMS_PER_BATCH // 1  # 512*1/gpus
            ...
            if int(storage.iter) >= self.warmup_distill:
                if self.semantic:
                    ...
                    gt_prototype_classes_s, gt_prototype_classes_l = self.predict_prototype(feature_pooled_s,
                                                                                            feature_pooled_l,
                                                                                            gt_classes)
