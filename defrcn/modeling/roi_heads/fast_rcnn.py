...
class FastRCNNOutputs(object):
    ...
    def __init__(
        ...
        if proposals[0].has("gt_boxes"):
            self.gt_boxes = box_type.cat([p.gt_boxes for p in proposals]) #[2048,4]
    ...
    def smooth_l1_loss(self):
        ...
        Returns:
            scalar Tensor
        """
        # print('proposal_tensor:{}'.format(len(self.proposals.tensor)))
        # print('gt_boxes_tensor:{}'.format(len(self.gt_boxes.tensor)))
        gt_proposal_deltas = self.box2box_transform.get_deltas(
            self.proposals.tensor, self.gt_boxes.tensor
        )  #(torch.Size([2048, 4]),[2048, 4])
...
class DoubleFastRCNNOutputLayers(nn.Module):
    ...
    def forward(self, x_s, x_l):
        ...
        proposal_deltas = self.bbox_pred(x_l)  #[2048,256]

        if self._do_cls_dropout:
            x_s = F.dropout(x_s, self._dropout_ratio, training=self.training)
        scores = self.cls_score(x_s)  #[2048,65]
