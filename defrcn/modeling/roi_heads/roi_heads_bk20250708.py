...
class CommonalityROIHeads(ROIHeads):
    ...
    def __init__(self, cfg, input_shape):
        ...
        elif self.num_classes in [60, 64, 80]:
            self.novel_index = [0, 1, 2, 3]
            self.novel_index2 = [0, 1, 2, 3, 4, 5, 6, 8, 14, 15, 16, 17, 18, 19, 39, 56, 57, 58, 60, 62]
        self.base_index = []
    ...
    def _shared_roi_transform(self, features, boxes):
        x = self.pooler(features, boxes) #torch.Size([8192, 1024, 7, 7])
        # print('pooler:', x.size())
        x = self.res5(x) #torch.Size([8192, 2048, 4, 4])
        # print('res5:', x.size())
        return x
    ...
    def forward(self, images, features, proposals, targets=None):
        ...
        #     [features[f] for f in self.in_features], proposal_boxes
        # )
        # feature_pooled = box_features.mean(dim=[2, 3])  # pooled to 1x1 torch.Size([8192, 2048])

        gt_boxes = [x.bboxes for x in targets]
        ...
        for feat, cls in zip(feature_pooled, all_classes):
            cls_idx = int(cls.item())  # 类别索引
            dict_cls[cls_idx].append(feat.detach().cpu()) 
            
        # #保存
        # out_file = open('/root/emtk_coal/selftest/coal_clsdist/30shot_prec.pkl','wb')  #(10, 101, 80, 4, 3)
        save_path = '/root/emtk_coal/selftest/coal_clsdist/{}.pkl'.format(img_id)
        with open(save_path, 'wb') as f:
            pickle.dump(dict_cls, f)
        ...
        outputs = FastRCNNOutputs(
            self.box2box_transform,
            pred_class_logits, #[2048，65]
            pred_proposal_deltas, #[2048,256]
            proposals,
        ...
        if self.training:          
            ...
            if self.memory: #更新队列
                with torch.no_grad():
                    gt_classes = outputs.gt_classes #[2048]
