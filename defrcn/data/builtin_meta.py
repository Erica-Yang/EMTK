...
# eval coco——coal时用：
def _get_coco_instances_meta():
    thing_ids = [k["id"] for k in COCO_CATEGORIES2 if k["isthing"] == 1]
    thing_colors = [k["color"] for k in COCO_CATEGORIES2 if k["isthing"] == 1]
    # assert len(thing_ids) == 80, len(thing_ids)
    # Mapping from the incontiguous COCO category id to an id in [0, 79]
    thing_dataset_id_to_contiguous_id = {k: i for i, k in enumerate(thing_ids)}
    thing_classes = [k["name"] for k in COCO_CATEGORIES2 if k["isthing"] == 1]
    ret = {
        "thing_dataset_id_to_contiguous_id": thing_dataset_id_to_contiguous_id,
        "thing_classes": thing_classes,
        "thing_colors": thing_colors,
...
def _get_coco_fewshot_instances_meta():
    ...
    novel_classes2 = [
        k["name"] for k in COCO_NOVEL_CATEGORIES2 if k["isthing"] == 1
    ]
    base_categories = [
        k for k in COCO_CATEGORIES2 if k["isthing"] == 1 and k["name"] not in novel_classes2
    ]
    # base_categories = [
    #     k for k in COCO_CATEGORIES if k["isthing"] == 1
    # ]
